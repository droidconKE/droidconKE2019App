...
* day_one_2019 - This collection contains a list of documents which have sessions which are for day one. Each day one session has the following fields:
  ...
  * start_status - string value for indicating whether a session has been started or not. Must either be "notStarted" or "sessionEnded"
  * stage - string value for the room the session will be at. It must be either of the following : MainHall ,Room1 , Room2 and Room3. The values are case-sensitive.
  
* day_two_2019 - This collection contains a list of documents which have sessions which are for day two.The fields in each document are similar to the one listed in day one. 
* event_feedback_2019 - This collection contains a list of documents which have the feedback from users for the whole event. Each feedback document contains the following field:
...
* session_feedback_2019 - This collection contains a list of documents which have the feedback from users for a particular session event. Each feedback document contains the following field:
...
* speakers2019 - This collection contains a list of documents which have the speaker details. Each speaker document has the following fields:
...
* sponsors_2019 - This collection contains a list of documents which have the sponsor details. Each sponsor document has the following fields:
...
* starred_sessions - This collection has a list of starred sessions documents. Each document has the following fields:
  ...
  * slug - string representing the notification slug for a session
 * reserved_seats - This collection has a list of people who have reserved their seats for a session. Each document has the following fields:
  * day_number - a string representing the day number of the session
  * session_id - string id for the session
  * session_title -  string representing the notification slug for a session
  * user_id - string id of the user starring the session
 * announcements - This collection has a list of annoucnements before,during and after tne event. Each document has the following fields:
  * description - a string representing the content of the announcement
  * title - string representing the title of the announcement  
